...
@section('content')
    ...
    <section class="content">
                    ...
                    <div class="box-body">
                        <div class="row">
                            <div class="col-lg-4">
                                ...
                                {!! Form::text('outside_order_list_filter_date_range', null, [
                                    'placeholder' => __('lang_v1.select_a_date_range'),
                                    'class' => 'form-control bookings_list_filter_date_range',
                                    'readonly',
                                ]) !!}
                            </div>
                            <div class="col-lg-4">
                                {!! Form::label('service', __('lang_v1.service')) !!}
                                {!! Form::select('category', $scategories, null, ['class' => 'form-control service_filter']) !!}
                            </div>
                            <div class="col-lg-4">
                                {!! Form::label('time', __('lang_v1.time')) !!}
                                <select name="time" class="form-control time_filter">
        ...
        <!--edit modal-->
        <div class="modal fade edit_booking" tabindex="-1" role="dialog" aria-labelledby="gridSystemModalLabel">
        </div>
        <!--end-->
    </section>
    <!-- /.content -->
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('.edit_booking').on('shown.bs.modal', function(e) {
                $('form#edit_booking_form #start_time').datetimepicker({
                    format: moment_date_format + ' ' + moment_time_format,
                    ignoreReadonly: true
                });
                $('form#edit_booking_form').on('submit', (e) => {
                    e.preventDefault();
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            $(e.target).find('button[type="submit"]').attr('disabled',
                                false);
                        }
                    });
                });
            ...
            $('.bookings_list_filter_date_range').daterangepicker(
                dateRangeSettings,
                function(start, end) {
                    $('.bookings_list_filter_date_range').val(start.format(moment_date_format) + ' ~ ' + end
            ...
            todays_bookings_table = $('#todays_bookings_table').DataTable({
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        var start = $('.bookings_list_filter_date_range').data('daterangepicker')
                            .startDate.format('YYYY-MM-DD');
                        var end = $('.bookings_list_filter_date_range').data('daterangepicker').endDate
                            .format('YYYY-MM-DD');
            ...
            $('.service_filter').on('change', (e) => {
                category = $(e.target).val();
                todays_bookings_table.ajax.reload();

            })
            $('.time_filter').on('change', (e) => {
                timing = $(e.target).val();
                todays_bookings_table.ajax.reload();
